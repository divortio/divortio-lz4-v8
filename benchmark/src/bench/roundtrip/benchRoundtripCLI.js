/**
 * benchmark/src/bench/roundtrip/benchRoundtripCLI.js
 */
...
async function main() {
    try {
        ...
        if (config.isHelp) {
            console.log("Usage: node benchRoundtripCLI.js -l <lib> [-l <lib2>] -i <input> [-s 5] [-w 2]");
            process.exit(0);
        }
        ...
        const bench = new BenchRoundtripInProc(libraries, inputFile, config.samples, config.warmups);
