/**
 * benchmark/src/bench/decompress/benchDecompressFilesCLI.js
 */
...
async function main() {
    try {
        ...
        if (config.isHelp) {
            console.log("Usage: node benchDecompressFilesCLI.js -l <lib> [-l <lib2>] -i <input> ... [-s 5] [-w 2]");
            process.exit(0);
        }
        ...
        const bench = new BenchDecompressFilesInProc(libraries, inputFiles, config.samples, config.warmups);
